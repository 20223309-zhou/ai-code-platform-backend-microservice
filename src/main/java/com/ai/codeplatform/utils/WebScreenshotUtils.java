...
public class WebScreenshotUtils {
    ...
    public static void cleanupTempFiles(){
        String cleanPath = System.getProperty("user.dir") + File.separator + "tmp" + File.separator + "screenshots";
        Path path = Paths.get(cleanPath);
        boolean result = FileUtil.clean(path.toFile());
        if (result){
            log.info("清理临时文件成功: {}", cleanPath);
        }
        if (!result){
            log.error("清理临时文件失败: {}", cleanPath);
        }
